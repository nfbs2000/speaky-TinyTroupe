...
release `0.7.0`을 읽었다. 공개 글쓰기 실험과 한국어 문서는 그 위에 더해진 별도
교육 계층이다.
...
[TinyTroupe Writing Lab](../education/)은 TinyTroupe의 persona simulation 철학을
Claude Agent SDK의 실제 child agent 글쓰기 팀에 적용한 별도 실험이다. 그 실험에서
`TinyPerson`은 simulated reader이고 Claude child는 실제로 작업을 위임받은 worker다.
두 존재를 같은 agent라고 부르지 않는다.
